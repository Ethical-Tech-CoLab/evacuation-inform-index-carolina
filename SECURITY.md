...
The Evacuation Inform Index (EII) — a crisis-severity index built on INFORM and open crisis reporting, published as a static site over a cached data snapshot.
...
Three CDN `<script>` tags are currently loaded without a Subresource Integrity attribute. No fix is open yet.
